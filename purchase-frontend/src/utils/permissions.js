...
export const hasPermission = (user, permissionCode) => {
  ...
  return permissions.some((code) => normalizePermissionCode(code) === normalized);
};

export const hasAnyPermission = (user, permissionCodes = []) =>
  Array.isArray(permissionCodes) && permissionCodes.some((code) => hasPermission(user, code));

export const hasAllPermissions = (user, permissionCodes = []) =>
  Array.isArray(permissionCodes) && permissionCodes.every((code) => hasPermission(user, code));