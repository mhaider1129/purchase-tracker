...
import {
  getRequestProgress,
  sortAssignedRequests,
} from "../utils/assignedRequests";
...
const evaluateCompletionState = (request = {}, itemsOverride = null) => {
  ...
  const hasRecordedCost =
    !Number.isNaN(numericSavedCost) && numericSavedCost >= 0;
...
const SummaryBadge = ({ label, value, tone = "default" }) => (
  <div
    className={`rounded-xl px-3.5 py-3 text-sm flex flex-col ${
...
const AssignedRequestsPage = () => {
  ...
  const [manualStatusForm, setManualStatusForm] = useState({
    status: "completed",
    comment: "",
  });
  ...
  const fetchItems = async (requestId) => {
    ...
    try {
      ...
      setRequests((prev) =>
        prev.map((req) => {
          if (req.id === requestId) {
            ...
            computedState = evaluateCompletionState(
              updatedRequest,
              fetchedItems,
            );
      ...
      setAutoTotals((prev) => ({
        ...prev,
        [requestId]: summary.items_total_cost,
      }));
      if (computedState) {
        setCompletionStates((prev) => ({
          ...prev,
          [requestId]: computedState,
        }));
      }
    } catch (err) {
      ...
      navigate("/request-submitted", {
        state: {
          title: tr(
            ...
            "Request Action Completed Successfully!",
          ),
          message,
          requestId,
          requestType:
            selectedRequest?.request_type ||
            tr("completionScreen.requestType", "Request"),
  ...
  const handleCostChange = (requestId, value) => {
    ...
    const normalizedCost =
      Number.isNaN(parsedCost) || parsedCost < 0 ? null : parsedCost;
  ...
  const submitManualStatus = async (event) => {
    ...
    try {
      await axios.put(
        `/requested-items/${manualStatusItem.id}/procurement-status`,
        {
  ...
  const handleSaveTotalCost = async (requestId) => {
    ...
    if (Number.isNaN(cost) || cost < 0) {
      alert(
        tr("alerts.invalidCost", "Enter a valid total cost of zero or more."),
      );
      return;
    }
    ...
    try {
      ...
      if (updatedRequestRef) {
        ...
        setCompletionStates((prev) => ({
          ...prev,
          [requestId]: evaluateCompletionState(
            updatedRequestRef,
            itemsOverride,
          ),
        }));
    ...
    try {
        ...
        prev.map((req) =>
          req.id === requestId
            ? {
                ...req,
                print_count:
                  printCount ?? request?.print_count ?? req.print_count,
              }
            : req,
        ),
      );
      ...
      if (!printWindow) {
        alert(
          tr(
            "alerts.popupBlocked",
            "Please enable popups to print the request.",
          ),
        );
        return;
      }
      ...
      const detailFields = [
        {
          label: tr("print.requestId", "Request ID"),
        ...
        {
          label: tr("print.department", "Department"),
          value: request?.department_name,
        },
        {
          label: tr("print.requester", "Requester"),
          value: getRequesterDisplay(request || {}),
        },
        {
          label: tr("print.createdOn", "Created On"),
          value: formatPrintableDate(request?.created_at),
        },
        {
          label: tr("print.neededBy", "Needed By"),
          value: formatPrintableDate(request?.needed_by),
        },
        {
          label: tr("print.printCount", "Print Count"),
          value: printCount ?? request?.print_count,
        },
        {
          label: tr("print.finalApproval", "Final Approval"),
          value: finalApproval,
        },
      ]
        .map(({ label, value }) => ({
          label,
          value: formatPrintableValue(value),
        }))
  ...
  const handleGenerateDoc = async (requestId, type) => {
    try {
      ...
      console.error(
        `❌ Error generating ${type.toUpperCase()} document for request ${requestId}:`,
        err,
      );
      const serverMessage =
        err?.response?.data?.message ||
      ...
      alert(
        serverMessage
          ? tr(
              "alerts.generateDocumentFailedWithReason",
              `Failed to generate document: ${serverMessage}`,
            )
  ...
  const handleDownloadAttachment = async (attachment) => {
    ...
    try {
      ...
      alert(
        tr(
          "alerts.attachmentDownloadFailed",
  ...
  const handleAutoPurchaseAll = async (requestId) => {
    ...
    if (!items.length) {
      alert(
        tr(
          "alerts.noItemsForBulkUpdate",
          "No items available to update for this request.",
        ),
      );
      return;
    }
    ...
    const updatableItems = items.filter((item) => {
      ...
      return (
        item.supports_procurement_events !== false &&
        item.id &&
    ...
    try {
      for (const item of updatableItems) {
        ...
        const remainingQty = Number(
          item.remaining_quantity ?? Math.max(requestedQty - purchasedQty, 0),
        );
        ...
        await axios.post(
          `/requests/${requestId}/items/${item.id}/procurement-events`,
          {
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter((request) => {
      if (
        typeFilter !== "all" &&
        String(request?.request_type || "") !== typeFilter
      ) {
        return false;
      }
      ...
      if (
        ...
        String(request?.department_name || "") !== departmentFilter
      ) {
        return false;
      }
      ...
      const completionState =
        completionStates[request.id] || evaluateCompletionState(request);
      if (completionFilter === "ready" && !completionState.canComplete) {
        return false;
      }

      if (
        completionFilter === "needs_attention" &&
  ...
  const pageKpis = useMemo(() => {
    ...
    const urgentRequests = filteredRequests.filter((req) =>
      Boolean(req?.is_urgent),
    ).length;
    const readyToComplete = filteredRequests.filter(
      (req) => completionStates[req.id]?.canComplete,
    ).length;
    const pendingCompletion = Math.max(totalRequests - readyToComplete, 0);

    return [
      {
        label: tr("kpis.totalRequests", "Total Assigned Requests"),
        value: totalRequests,
      ...
      {
        label: tr("kpis.urgentRequests", "Urgent Requests"),
      ...
      {
        label: tr("kpis.readyToComplete", "Ready to Complete"),
        value: readyToComplete,
        icon: <CheckCircle2 size={20} />,
        accent: "bg-emerald-500",
        iconClass: "bg-emerald-50 text-emerald-600",
      },
      {
        label: tr("kpis.pendingCompletion", "Needs Attention"),
  ...
  return (
    <PageShell
      ...
      description={tr(
        "description",
        "Track procurement progress, update item statuses, and complete requests once all items are finalized.",
      )}
      actions={
        <button
          type="button"
          className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm">
          <p className="text-sm text-slate-600">
            {tr("empty", "No requests assigned to you.")}
          </p>
        </div>
      ) : (
        <>
          <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center gap-3 border-b border-slate-100 pb-4">
              ...
              <div>
                <h2 className="text-sm font-bold text-slate-900">
                  {tr("filters.title", "Find your next request")}
                </h2>
                <p className="text-xs text-slate-500">
                  {tr(
                    "filters.helper",
                    "Search, filter, and prioritize your assigned procurement work.",
                  )}
                </p>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-6">
              <div className="lg:col-span-2">
                <label
                  htmlFor="assigned-request-search"
                ...
                <input
                  ...
                  placeholder={tr(
                    ...
                    "Search by ID, requester, project, department, type, or justification",
                  )}
                  className="w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm transition focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                />
              </div>
              <div>
                <label
                  htmlFor="assigned-request-type-filter"
                  className="mb-1 block text-sm font-medium text-slate-700"
                >
              ...
              <div>
                <label
                  htmlFor="assigned-request-department-filter"
                  className="mb-1 block text-sm font-medium text-slate-700"
                >
                ...
                >
                  <option value="all">
                    {tr("filters.allDepartments", "All departments")}
                  </option>
              ...
              <div>
                <label
                  htmlFor="assigned-request-completion-filter"
                  className="mb-1 block text-sm font-medium text-slate-700"
                >
                ...
                >
                  <option value="all">
                    {tr("filters.allCompletion", "All completion states")}
                  </option>
                  <option value="ready">
                    {tr("filters.readyToComplete", "Ready to complete")}
                  </option>
                  <option value="needs_attention">
                    {tr("filters.needsAttention", "Needs attention")}
                  </option>
                </select>
              </div>
              <div>
                <label
                  htmlFor="assigned-request-urgency-filter"
                  className="mb-1 block text-sm font-medium text-slate-700"
                >
                ...
                >
                  <option value="all">
                    {tr("filters.allUrgency", "All urgency levels")}
                  </option>
                  <option value="urgent">
                    {tr("filters.urgentOnly", "Urgent only")}
                  </option>
                  <option value="standard">
                    {tr("filters.standardOnly", "Standard only")}
                  </option>
                </select>
              </div>
              <div>
                <label
                  htmlFor="assigned-request-sort"
                  className="mb-1 block text-sm font-medium text-slate-700"
                >
                ...
                >
                  <option value="priority">
                    {tr("filters.priorityFirst", "Priority first")}
                  </option>
                  <option value="newest">
                    {tr("filters.newestFirst", "Newest first")}
                  </option>
                  <option value="oldest">
                    {tr("filters.oldestFirst", "Oldest first")}
                  </option>
                  <option value="progress">
                    {tr("filters.mostProgress", "Most progress")}
                  </option>
                </select>
              </div>
            </div>
            <div className="mt-4 flex flex-col gap-2 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
              <span>
                {tr(
                  "filters.resultCount",
                  "Showing {{shown}} of {{total}} assigned requests",
                  {
          ...
          {filteredRequests.length === 0 ? (
            ...
            visibleRequests.map((request) => {
              ...
              const completionState = completionStates[request.id] || {
                canComplete: false,
                missingCost: false,
              ...
              const containerClasses = [
                "mb-6 overflow-hidden rounded-2xl border bg-white shadow-sm transition duration-200 hover:shadow-md",
                isUrgent
              ...
              return (
                <div key={request.id} className={containerClasses}>
                  <div
                    className={`h-1.5 ${isUrgent ? "bg-rose-500" : completionState.canComplete ? "bg-emerald-500" : "bg-blue-600"}`}
                    aria-hidden="true"
                  />
                    ...
                    <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isUrgent && (
                            <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 text-rose-700 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide">
                              <AlertTriangle size={11} aria-hidden="true" />
                              {tr("requestCard.urgent", "Urgent")}
                            </span>
                          )}
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${completionState.canComplete ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-800"}`}
                          >
                            {completionState.canComplete ? (
                        ...
                        <h2 className="mt-3 truncate text-xl font-bold text-slate-900">
                          {request.request_type ||
                            tr("requestCard.untitled", "Purchase request")}
                        </h2>
                        <p className="mt-1 text-sm text-slate-500">
                          {request.project_name ||
                            tr("requestCard.noProject", "No project specified")}
                        </p>
                        ...
                        {request.justification && (
                          <p className="mt-4 line-clamp-2 max-w-3xl border-l-2 border-slate-200 pl-3 text-sm leading-6 text-slate-600">
                            {request.justification}
                          </p>
                        )}
                      </div>

                      <div className="grid shrink-0 grid-cols-2 gap-2 sm:flex sm:flex-wrap lg:max-w-xs lg:justify-end">
                        <button
                          onClick={() => handleMarkAsCompleted(request.id)}
                          className={`col-span-2 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-white transition sm:order-last sm:w-full ${
                            completionState.canComplete
                        ...
                        >
                          <FileText size={15} /> Workspace
                        </Link>
                        <button
                          onClick={() => toggleExpand(request.id)}
                          className="col-span-2 inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 sm:col-auto"
                        >
                        ...
                        <Link
                          ...
                          className="inline-flex items-center justify-center gap-1 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                        >
                          {tr("requestCard.viewPo", "View PO")}{" "}
                          <ArrowRight size={14} />
                        </Link>
                        <button
                          className="col-span-2 inline-flex items-center justify-center gap-1 rounded-lg px-3 py-2 text-sm font-semibold text-blue-600 hover:bg-blue-50 sm:col-auto"
                          onClick={() => toggleApprovals(request.id)}
                          disabled={loadingApprovalsId === request.id}
                        >
                          <ChevronDown
                            size={14}
                            className={`transition ${expandedApprovalsId === request.id ? "rotate-180" : ""}`}
                          />
                        ...
                        {showCompletionHints && (
                          <div className="col-span-2 text-xs text-left text-rose-600 space-y-1 sm:w-full">
                      ...
                      <div className="mt-6 border-t border-slate-200 pt-6">
                          ...
                          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                            <div className="flex-1">
                              ...
                              <AmountInput
                                ...
                                onChange={(e) =>
                                  handleCostChange(request.id, e.target.value)
                                }
                              ...
                              {autoTotal !== null && (
                                <div className="mt-1 flex items-center justify-between text-xs text-gray-500">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleCostChange(request.id, autoTotal)
                                    }
                        ...
                        {loadingItems ? (
                          <p className="text-gray-500">
                            {tr("items.loading", "Loading items...")}
                          </p>
                        ) : items.length === 0 ? (
                          <p className="text-gray-500">
                            {tr(
                          ...
                          <>
                              ...
                              <div className="mb-6 rounded-lg border border-emerald-200 bg-emerald-50 p-4">
                                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleAutoPurchaseAll(request.id)
                                    }
                                    disabled={
                            ...
                            <div className="mb-4 flex flex-col gap-2 rounded-lg border border-slate-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                              <div>
                                ...
                                <p className="text-xs text-slate-500">
                                  {tr(
                                    "items.sort.helper",
                                    "Sorting only changes this view and does not update the saved request.",
                                  )}
                                </p>
                              </div>
                              <button
                                type="button"
                                onClick={() =>
                                  setSortItemsAlphabetically((prev) => !prev)
                                }
                            ...
                            {itemSections.map(
                              ({ key, title, description, tone, empty }) => {
                                const sectionItems = getDisplayItems(
                                  groupedItems[key] || [],
                                  sortItemsAlphabetically,
                                );
                                if (
                                    ...
                                    {sectionItems.length === 0 ? (
                                      ...
                                      sectionItems.map((item, idx) => {
                                        ...
                                        return (
                                          <div
                                            key={item.id || idx}
                                            className="mb-3 rounded-xl border border-slate-200 bg-white"
                                          >
                                            {canManuallyClose && (
                        ...
                        <div className="mt-6">
                          <h3 className="font-semibold mb-2">
                            {tr("attachments.title", "Attachments")}
                          </h3>
                          {loadingAttachments ? (
                            <p className="text-gray-500">
                              {tr(
                            ...
                            <p className="text-gray-500">
                              {tr("attachments.empty", "No attachments found.")}
                            </p>
                          ) : (
                        ...
                        <div className="mt-6 border-t border-slate-200 pt-6">
                          <h3 className="font-semibold mb-3">
                            {tr("documents.title", "Generate Document")}
                          </h3>
                          <div className="flex flex-wrap gap-3">
                            {["rfp", "rfi", "rfq"].map((type) => (
                              <button
                                key={type}
                                onClick={() =>
                                  handleGenerateDoc(request.id, type)
                                }
          ...
          <div className="w-full max-w-lg rounded-xl bg-white p-5 shadow-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {manualStatusItem.item_name}
                </p>
              </div>
              <button
                type="button"
                onClick={closeManualStatusModal}
            ...
            <form onSubmit={submitManualStatus} className="mt-4 space-y-4">
              ...
              <label className="block text-sm font-semibold text-slate-700">
                ...
                >
                  <option value="completed">
                    {tr(
                      "items.manualClose.completedOption",
                      "Completed / Close remaining quantity",
                    )}
                  </option>
                  <option value="not_procured">
                    {tr(
                      "items.manualClose.notProcuredOption",
                      "Not Procured / Will not be purchased",
                    )}
                  </option>
                  <option value="canceled">
                    {tr("items.manualClose.canceledOption", "Canceled")}
                  </option>
                </select>
              </label>
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {savingManualStatus
                    ? tr("items.manualClose.saving", "Saving…")
                    : tr("items.manualClose.save", "Save status")}
                </button>
              </div>
            </form>
          </div>
