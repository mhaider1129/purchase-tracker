...
const formatPercentInput = (value) => {
  ...
  return numeric > 0 && numeric <= 1
    ? String(Number((numeric * 100).toFixed(4)))
    : String(value);
};
...
const buildComparisonRows = ({
  ...
  const headers = [
    ...
    ...offers.flatMap((offer) => [
      `${offer.offer_name} pricing method`,
  ...
  const bodyRows = tests.map((test) => [
    test.test_name,
    Number(test.expected_monthly_volume || 0),
    Number(test.expected_monthly_volume || 0) * 12,
    ...offers.flatMap((offer) => {
  ...
  const totalRow = [
    "TOTAL",
    tests.reduce(
      (sum, test) => sum + Number(test.expected_monthly_volume || 0),
      0,
    ),
    tests.reduce(
      (sum, test) => sum + Number(test.expected_monthly_volume || 0) * 12,
      0,
    ),
    ...offers.flatMap((offer) => {
      const annualTotal = tests.reduce((sum, test) => {
        const key = `${offer.id}:${test.id}`;
        const current = {
          ...(costByPair[key] || {}),
          ...(costDrafts[key] || {}),
  ...
  return [headers, ...bodyRows, totalRow];
};

const calculateWastePercentage = ({
  tests_per_kit,
  usable_tests_per_kit,
  ...
  const waste = Math.max(
    0,
    Math.min(100, (1 - usableTests / denominator) * 100),
  );
...
const normalizeCostDraft = (draft) => ({
  ...draft,
  expected_waste_percentage: formatPercentInput(
    draft.expected_waste_percentage,
  ),
...
const ProcurementEvaluationDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    window.location.hash === "#recommendation" ? "Recommendation" : "Overview",
  );
  ...
  const costByPair = useMemo(
    () =>
      Object.fromEntries(
        costs.map((item) => [`${item.offer_id}:${item.test_id}`, item]),
      ),
    [costs],
  );
  const cheapestByTest = useMemo(() => {
    const map = {};
    tests.forEach((test) => {
      const annualCosts = offers
        .map((offer) => costByPair[`${offer.id}:${test.id}`])
        .filter(Boolean)
        .map((item) => Number(item.annual_test_cost || 0))
  ...
  const exportComparisonExcel = () => {
    ...
    saveAs(
      blob,
      `procurement-comparison-${id}-${new Date().toISOString().split("T")[0]}.xls`,
    );
  };
  ...
  const createCriteria = async (event) => {
    event.preventDefault();
    const response = await procurementEvaluationsApi.createCriteria(
      id,
      criteriaForm,
    );
  ...
  const saveCostMatrix = async (keysToSave = null) => {
    const draftEntries = Object.entries(costDrafts).filter(
      ([key]) => !keysToSave || keysToSave.includes(key),
    );
    const items = draftEntries.map(([key, value]) => {
      ...
      return {
        ...
        expected_monthly_volume: test?.expected_monthly_volume || 0,
        ...value,
      };
    });
  ...
  const updateCostDraft = (key, current, field, value) => {
    setCostDrafts((previous) => {
      const nextDraft = normalizeCostDraft({
        ...current,
        ...(previous[key] || {}),
        [field]: value,
      });
  ...
  const loadCoverage = async () => {
    const response = await procurementEvaluationsApi.coverage(id);
    setCoverage(response.data);
  };
  const loadItemComparison = async () => {
    const response = await procurementEvaluationsApi.itemComparison(id);
    setItemComparison(response.data || []);
  };
  const previewImport = async () => {
    const response = await procurementEvaluationsApi.previewImport(id, {
      text: importText,
    });
    setImportPreview(response.data);
  };
  const confirmImport = async () => {
    if (!importOfferId) return;
    await procurementEvaluationsApi.confirmImport(id, importOfferId, {
      text: importText,
      option: importOption,
      columnMap: importPreview?.columnMap,
    });
    setImportText("");
  ...
  const finalize = async () => {
    ...
    if (!selected) return;
    await procurementEvaluationsApi.finalize(id, {
      selected_offer_id: selected.offer_id,
      recommendation_summary: summaryDraft,
    });
    await loadAll();
  };

  useEffect(() => {
    if (activeTab === "Sensitivity Analysis")
      loadSensitivity().catch((err) =>
        setError(err.response?.data?.message || "Failed to load sensitivity."),
      );
    if (activeTab === "Recommendation")
      loadRecommendation().catch((err) =>
        setError(
          err.response?.data?.message || "Failed to load recommendation.",
        ),
      );
  ...
  if (!evaluation)
    return (
      <div className="p-6 text-slate-600">Loading procurement evaluation…</div>
    );
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                {evaluation.evaluation_type} · {evaluation.category} ·{" "}
                {evaluation.evaluation_period_years} year TCO ·{" "}
                {evaluation.currency}
              </p>
            </div>
        ...
        <div className="flex gap-2 overflow-x-auto rounded-2xl bg-white p-2 shadow-sm ring-1 ring-slate-200">
          {tabs.map((tab) => (
            <button
              ...
              className={`whitespace-nowrap rounded-xl px-4 py-2 text-sm font-semibold ${activeTab === tab ? "bg-indigo-600 text-white" : "text-slate-700 hover:bg-slate-100"}`}
            >
              {tab}
            </button>
          ))}
        ...
        {activeTab === "Overview" && (
          <section className="grid gap-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 md:grid-cols-2">
            {[
              ...
              [
                "Linked request",
                evaluation.request_id ? `#${evaluation.request_id}` : "Manual",
              ],
              ["Department", evaluation.department_id || "—"],
              ["Evaluation type", evaluation.evaluation_type],
              ["Evaluation period years", evaluation.evaluation_period_years],
              ["Annual growth rate", evaluation.expected_annual_growth_rate],
            ...
            {!readOnly && (
              ...
              >
                ...
                <input
                  ...
                  placeholder="Model"
                  onChange={(e) =>
                    setOfferForm({ ...offerForm, model_name: e.target.value })
                  }
                />
                <select
                  className={inputClass}
                  value={offerForm.pricing_model}
                  onChange={(e) =>
                    setOfferForm({
                      ...offerForm,
              ...
              {offers.map((offer) => (
                ...
                >
                  ...
                  <p className="text-sm text-slate-600">
                    {offer.supplier_name} · {offer.manufacturer_name || "—"}{" "}
                    {offer.model_name || ""}
                  </p>
                  <dl className="mt-3 grid grid-cols-2 gap-2 text-sm">
                    <dt>Pricing</dt>
                    <dd>{offer.pricing_model}</dd>
                    <dt>Initial</dt>
                    <dd>
                      {money(
                        Number(offer.device_price) +
                          Number(offer.installation_cost) +
                          Number(offer.training_cost) +
                          Number(offer.shipping_cost) +
                          Number(offer.customs_cost) +
                          Number(offer.other_initial_cost) -
                          Number(offer.device_discount_value),
                      )}
                    </dd>
                    <dt>Warranty</dt>
                    ...
                    {offer.pricing_model === "REAGENT_RENTAL" && (
                      <>
                        <dt>Min amount</dt>
                        <dd>{money(offer.minimum_annual_commitment_amount)}</dd>
                        <dt>Min tests</dt>
                        <dd>{money(offer.minimum_annual_commitment_tests)}</dd>
                      </>
                    )}
                  </dl>
                </div>
        ...
        {[
          ...
          <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            {!readOnly && (
              <form onSubmit={createTest} className="flex flex-wrap gap-3">
                <input
                  className={inputClass}
                ...
                <input
                  ...
                  value={testForm.expected_monthly_volume}
                  onChange={(e) =>
                    setTestForm({
                      ...testForm,
                  ...
                  onClick={() =>
                    navigator.clipboard?.writeText(
                      costs
                        .map(
                          (c) =>
                            `${c.test_name},${c.offer_name},${c.annual_test_cost}`,
                        )
                        .join("\n"),
                    )
                  }
                  className="rounded-lg border px-4 py-2 font-semibold text-slate-700"
                >
                  Export CSV
                </button>
                <button
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border-separate border-spacing-0 text-xs">
                <thead>
                  <tr>
                    <th className="sticky left-0 bg-slate-100 p-2 text-left">
                      Test / Monthly / Annual Volume
                    </th>
                    {offers.map((offer) => (
                      <th
                        key={offer.id}
                        className="min-w-[280px] bg-slate-100 p-2 text-left"
                      >
                        {offer.offer_name}
                      </th>
                    ))}
                ...
                <tbody>
                    ...
                    <tr key={test.id}>
                      <td className="sticky left-0 border-t bg-white p-2 font-semibold">
                        ...
                        <span className="text-slate-500">
                          {money(test.expected_monthly_volume)} tests/month /{" "}
                          {money(Number(test.expected_monthly_volume) * 12)}{" "}
                          tests/year
                        </span>
                      </td>
                      {offers.map((offer) => {
                        ...
                        return (
                          <td
                            key={key}
                            className={`border-t p-2 align-top ${cheapest ? "bg-emerald-50" : ""}`}
                          >
                            <select
                              className={inputClass}
                            ...
                            >
                              <option>KIT_OWNERSHIP</option>
                              <option>PAY_PER_REPORTABLE</option>
                            </select>
                            {current.pricing_method === "PAY_PER_REPORTABLE" ? (
                              <div className="mt-2 grid grid-cols-2 gap-2">
                                <input
                                  className={inputClass}
                                  placeholder="Price/report"
                                  value={
                                    current.price_per_reportable_test || ""
                                  }
                                  onChange={(e) =>
                                    updateCostDraft(
                                      key,
                                      current,
                                      "price_per_reportable_test",
                                      e.target.value,
                                    )
                                  }
                                />
                                <label>
                                  <input
                                    type="checkbox"
                                    checked={Boolean(
                                      current.company_absorbs_waste,
                                    )}
                                    onChange={(e) =>
                                      updateCostDraft(
                                        key,
                                        current,
                                        "company_absorbs_waste",
                                        e.target.checked,
                                      )
                                    }
                                  />{" "}
                                ...
                                <label>
                                  <input
                                    type="checkbox"
                                    checked={Boolean(
                                      current.company_absorbs_repeats,
                                    )}
                                    onChange={(e) =>
                                      updateCostDraft(
                                        key,
                                        current,
                                        "company_absorbs_repeats",
                                        e.target.checked,
                                      )
                                    }
                                  />{" "}
                              ...
                              <div className="mt-2 grid grid-cols-2 gap-2">
                                {[
                                  ["kit_price", "Kit price"],
                                  ["tests_per_kit", "Tests/kit"],
                                  ["usable_tests_per_kit", "Usable tests"],
                                  ["expected_waste_percentage", "Waste %"],
                                  ["repeat_rate_percentage", "Repeat %"],
                                  ["qc_cost_per_kit", "QC/test"],
                                  [
                                    "calibrator_cost_per_kit",
                                    "Calibrator/test",
                                  ],
                                  [
                                    "fixed_consumable_cost_per_kit",
                                    "Fixed cons./test",
                                  ],
                                  ["other_kit_related_cost", "Other"],
                                ].map(([field, label]) => (
                                  <input
                                    key={field}
                                    className={inputClass}
                                    ...
                                    value={
                                      [
                                        ...
                                        ? formatPercentInput(current[field])
                                        : current[field] || ""
                                    }
                                    onChange={(e) =>
                                      updateCostDraft(
                  ...
                  <tr className="border-t-2 border-slate-300 bg-slate-100 font-bold">
                    <td className="sticky left-0 bg-slate-100 p-2">
                      Total annual consumption
                    </td>
                    {annualTotalsByOffer.map((offer) => (
        ...
        {activeTab === "TCO Analysis" && (
          <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            >
              Total weight: {criteriaWeightTotal} (should total 100)
            </p>
            {!readOnly && (
              <form onSubmit={createCriteria} className="flex flex-wrap gap-3">
                <input
                  className={inputClass}
                ...
                <input
                  className={inputClass}
                  placeholder="Group"
                  value={criteriaForm.criteria_group}
                  onChange={(e) =>
                    setCriteriaForm({
                      ...criteriaForm,
                      criteria_group: e.target.value,
                    })
                  }
                />
                <input
              ...
              <tbody>
                {criteria.map((item) => (
                  <tr key={item.id} className="border-t">
                    <td className="p-2 font-semibold">{item.criteria_name}</td>
                ...
                <tbody>
                  {offers.map((offer) => (
                    <tr key={offer.id} className="border-t">
                      <td className="p-2 font-semibold">{offer.offer_name}</td>
                      {criteria.map((c) => {
                        const key = `${offer.id}:${c.id}`;
                        const existing = scores.find(
                          (s) =>
                            Number(s.offer_id) === Number(offer.id) &&
                            Number(s.criteria_id) === Number(c.id),
                        );
                        return (
                          <td key={key} className="p-2">
                            <input
                              disabled={
                                readOnly || c.scoring_type === "automatic"
                              }
                              className={inputClass}
                              type="number"
                              min="0"
        ...
        {activeTab === "Results" && (
          <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div className="grid gap-4 md:grid-cols-3">
              {results.map((row) => (
                <div key={row.offer_id} className="rounded-xl border p-4">
                  ...
                  <p>
                    Score: {Number(row.final_weighted_score || 0).toFixed(2)}
                  </p>
                  {row.commitment_warning && (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {results.map((row) => (
                    <tr key={row.offer_id} className="border-t">
                      <td>{row.rank}</td>
                      <td>{row.offer_name}</td>
                      <td>{row.pricing_model}</td>
                      <td>{money(row.initial_cost)}</td>
                      <td>{money(row.annual_fixed_cost)}</td>
                      <td>{money(row.annual_variable_test_cost)}</td>
          ...
          <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {coverage?.offers?.map((offer) => (
                <div key={offer.offer_id} className="rounded-xl border p-4">
                  <h3 className="font-bold">{offer.offer_name}</h3>
                  <p className="text-2xl font-bold text-indigo-700">
                    {offer.coverage_percentage}%
                  </p>
                  <p>
                    {offer.covered_count}/{offer.required_count} required items
                  </p>
                  <p className="text-sm text-red-700">
                    Missing: {offer.missing_items?.join(", ") || "None"}
                  </p>
                </div>
              ))}
            </div>
            {coverage?.unavailable_items?.length ? (
              <p className="rounded bg-red-50 p-3 text-red-700">
                Unavailable: {coverage.unavailable_items.join(", ")}
              </p>
            ) : null}
          </section>
        )}
            ...
            <table className="min-w-full text-sm">
              <tbody>
                {itemComparison.map((item) => (
                  <tr key={item.test_name} className="border-t">
                    <td className="p-2 font-semibold">{item.test_name}</td>
                    <td>
                      Cheapest: {item.cheapest_supplier?.offer_name || "—"}
                    </td>
                    <td>
                      {item.savings_opportunities
                        ?.map(
                          (saving) =>
                            `${saving.offer_name}: ${money(saving.potential_saving)}`,
                        )
                        .join("; ") || "No savings gap"}
                    </td>
                  </tr>
        ...
        {activeTab === "Commercial Models" && !readOnly && (
          <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <h2 className="text-xl font-bold">
              Bulk Excel / CSV Import and Commercial Model Mapping
            </h2>
            <div className="flex flex-wrap gap-3">
              <select
                className={inputClass}
                value={importOfferId}
                onChange={(e) => setImportOfferId(e.target.value)}
              >
                <option value="">Company offer</option>
                {offers.map((offer) => (
                  <option key={offer.id} value={offer.id}>
                    {offer.offer_name}
                  </option>
            ...
            <textarea
              className="min-h-[150px] w-full rounded-lg border p-3"
              placeholder="Paste Excel/CSV rows with headers"
              value={importText}
            ...
            {importPreview && (
              <div className="rounded bg-slate-50 p-3 text-sm">
                <p>
                  Valid: {importPreview.valid_count}; Errors:{" "}
                  {importPreview.error_count}
                </p>
                <p>
                  Mapping:{" "}
                  {Object.entries(importPreview.columnMap || {})
        ...
        {activeTab === "Sensitivity Analysis" && (
          <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <button
              ...
              className="mb-4 rounded-lg border px-4 py-2 font-semibold"
            >
              Refresh Sensitivity
            </button>
            <div className="grid gap-4 md:grid-cols-2">
              {sensitivity.map((scenario) => (
                <div key={scenario.key} className="rounded-xl border p-4">
                  ...
                  <p
                    className={
                      scenario.recommendation_changes
                        ? "text-amber-700"
                        : "text-emerald-700"
                    }
                  >
                    Winner: {scenario.winner?.offer_name || "—"}{" "}
                    {scenario.recommendation_changes
                      ? "(recommendation changes)"
                      : ""}
                  </p>
                  {scenario.offers?.map((offer) => (
                    <p key={offer.offer_id} className="text-sm text-slate-600">
                      {offer.offer_name}: TCO {money(offer.tco_period_cost)}
                    </p>
                  ))}
                </div>
              ))}
        ...
        {activeTab === "Documents" && (
          <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            <p className="mt-2 text-slate-600">
              Attach quotations, brochures, technical specs, price lists,
              service terms, warranty documents, compliance certificates, and
              committee reports here when the shared document module is linked
              to procurement evaluations.
            </p>
            <input
              type="file"
        ...
        {activeTab === "Recommendation" && (
          <section className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            {recommendation && (
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-xl bg-slate-50 p-4">
                  ...
                  <p>
                    Lowest TCO:{" "}
                    {recommendation.lowest_tco_offer?.offer_name ||
                      recommendation.lowest_tco_offer?.offer_id ||
                      "—"}
                  </p>
                  <p>
                    Lowest average cost/test:{" "}
                    {recommendation.lowest_average_cost_offer?.offer_name ||
                      recommendation.lowest_average_cost_offer?.offer_id ||
                      "—"}
                  </p>
                  <p>
                    Best weighted score:{" "}
                    {recommendation.best_final_weighted_score_offer
                      ?.offer_name ||
                      recommendation.best_final_weighted_score_offer
                        ?.offer_id ||
                      "—"}
                  ...
                  <p>
                    Recommended:{" "}
                    {recommendation.final_recommended_offer?.offer_name ||
                      recommendation.final_recommended_offer?.offer_id ||
                      "—"}
                  </p>
                </div>
            ...
            <textarea
              className="min-h-[160px] w-full rounded-lg border border-slate-300 p-3"
              value={summaryDraft}
              onChange={(e) => setSummaryDraft(e.target.value)}
              placeholder="Editable recommendation summary before finalization"
            />
            <button
              onClick={finalize}
              disabled={readOnly}
              className="rounded-lg bg-indigo-600 px-4 py-2 font-semibold text-white disabled:bg-slate-300"
            >
              Finalize Selected Offer
            </button>
          </section>
        )}
      </div>
