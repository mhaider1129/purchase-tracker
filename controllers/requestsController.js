...
// Create request and dynamically assign approvers using approval_routes
const createRequest = async (req, res) => {
  ...
  try {
    ...
    // 5. Fetch approval route dynamically
    ...
    for (const route of routes) {
      const approverRes = await client.query(
        `SELECT id FROM users WHERE role = $1 AND department_id = $2 LIMIT 1`,
      ...
      if (approver) {
        await client.query(
          `INSERT INTO approvals (request_id, approver_id, approval_level, status)
           VALUES ($1, $2, $3, 'Pending')`,
          [request.id, approver.id, route.approval_level]
        );
      }
    }
...
await client.query(
  ...
  [request.id, 'Created', requester_id, justification]
    ...
    res.status(201).json({
      message: 'Request created with dynamic approval routing',
...
// Get request with items and approvals
...
// Get all requests with full filtering and pagination
const getAllRequests = async (req, res) => {
  ...
  try {
    ...
    // You can later add more roles here (e.g., SCM, COO)
    ...
    if (offset) {
      baseQuery += ` OFFSET $${values.length + 1}`;
      values.push(offset);
    }
    ...
    console.error('Error fetching filtered requests:', err.message);
    res.status(500).json({ error: 'Failed to fetch requests' });
  }
};

// Get requests pending approval for this user's role
...
module.exports.getRequestLogs = getRequestLogs;
