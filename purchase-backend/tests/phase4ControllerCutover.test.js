...
describe('live Phase 4 controller cutover', () => {
  test('createPurchaseOrder delegates award quantities to the canonical service', async () => {
    ...
    expect(mockCreatePurchaseOrderFromAwards).toHaveBeenCalledWith(expect.objectContaining({ requestId: 9, awardIds: [2], quantities: { 2: '60' }, actor: req.user }));
    expect(res.status).toHaveBeenCalledWith(201);
  });

  test('createPurchaseOrder rejects legacy arbitrary item pricing input', async () => {
    const next = jest.fn();
    await controller.createPurchaseOrder({ params: { requestId: '9' }, user: { id: 7, role: 'scm' }, body: { items: [{ unit_price: 1 }] } }, {}, next);
