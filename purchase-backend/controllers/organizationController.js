...
exports.bulkReconcile=wrap(async(req,res)=>{const requested=[...new Set((req.body.unitIds||[]).map(String))];if(!requested.length)return res.json({assigned:0,results:[]});const results=await service.transaction(async client=>{
  // Acquire every unit lock in a stable order before validating or writing. This
  // prevents a concurrent hierarchy/head mutation from invalidating the preview.
  const locked=new Map();
  for(const id of [...requested].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true})))locked.set(id,await reconciliation.lockedRow(id,req.user.institute_id,client));
  const selected=requested.map(id=>locked.get(id));
  const unsafe=requested.filter((id,index)=>!selected[index]?.safeToAssign);
  if(unsafe.length){const error=new Error('Bulk reconciliation changed and is no longer safe');error.statusCode=409;error.details={unsafeUnitIds:unsafe};throw error;}
  const assigned=[];for(const row of selected)assigned.push(await service.assignHeadWithClient(row.unitId,{userId:row.legacyCandidates.find(x=>x.is_active!==false).id,actorId:req.user.id,auditAction:'ORGANIZATION_HEAD_RECONCILED',reason:'Bulk safe legacy HOD reconciliation'},client));return assigned;
