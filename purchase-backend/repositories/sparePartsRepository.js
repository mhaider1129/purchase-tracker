...
class SparePartsRepository {
  ...
  async stockItem(id,instituteId) { const r=await this.db.query(`SELECT si.id,si.name,si.generic_item_id,si.approved_product_id,si.unit,
    COALESCE(sum(wsl.quantity),0) on_hand,COALESCE(sum(wsl.reserved_quantity),0) reserved,
    COALESCE(sum(COALESCE(wsl.quantity,0)-COALESCE(wsl.reserved_quantity,0)),0) available
    FROM stock_items si JOIN warehouse_stock_levels wsl ON wsl.stock_item_id=si.id JOIN warehouses w ON w.id=wsl.warehouse_id
    WHERE si.id=$1 AND w.institute_id=$2 GROUP BY si.id`,[id,instituteId]); return r.rows[0]||null; }
  async availableStockItems(instituteId,{generic_item_id,approved_product_id}={}) { const values=[instituteId],where=['w.institute_id=$1']; if(generic_item_id){values.push(generic_item_id);where.push(`si.generic_item_id=$${values.length}`);} if(approved_product_id){values.push(approved_product_id);where.push(`(si.approved_product_id IS NULL OR si.approved_product_id=$${values.length})`);} const r=await this.db.query(`SELECT si.id,si.name,si.brand,si.unit,si.generic_item_id,si.approved_product_id,
    COALESCE(sum(wsl.quantity),0) on_hand,COALESCE(sum(wsl.reserved_quantity),0) reserved,COALESCE(sum(COALESCE(wsl.quantity,0)-COALESCE(wsl.reserved_quantity,0)),0) available
    FROM stock_items si JOIN warehouse_stock_levels wsl ON wsl.stock_item_id=si.id JOIN warehouses w ON w.id=wsl.warehouse_id
    WHERE ${where.join(' AND ')} GROUP BY si.id ORDER BY si.name LIMIT 100`,values); return r.rows; }
  ...
  async compatibilityById(id,spareId,lock=false) { const r=await this.db.query(`SELECT * FROM spare_part_equipment_compatibility WHERE id=$1 AND spare_part_id=$2${lock?' FOR UPDATE':''}`,[id,spareId]); return r.rows[0]||null; }
