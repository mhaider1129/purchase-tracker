...
const repo=existing=>{const inserted=[];const activities=[];const profiles=[];return{inserted,activities,profiles,withTransaction:fn=>fn({findActiveByRequestedItem:async()=>existing,insertCase:async x=>(inserted.push(x),{id:9,opened_at:'2026-01-01',...x}),provisionPriorityProfile:async x=>(profiles.push(x),{...x,coverage_status:'NEEDS_ASSESSMENT'}),insertActivityIdempotent:async x=>activities.push(x)})}};

describe('case/activity governance',()=>{
 test('approved item creates one case and neutral priority profile; retry provisions idempotently',async()=>{const r=repo();expect((await ensureCaseForRequestedItem({repository:r,requestedItem:item,actorId:1})).case_status).toBe('ITEM_IDENTITY_RESOLUTION');expect(r.inserted).toHaveLength(1);expect(r.profiles).toEqual([{procurement_case_id:9,institute_id:2,department_id:4}]);const old={id:9};const retry=repo(old);expect(await ensureCaseForRequestedItem({repository:retry,requestedItem:item,actorId:1})).toBe(old);expect(retry.profiles).toHaveLength(1)});
...
describe('complexity',()=>{
 ...
 test('assessment persists its validated reason inside the transaction without changing scoring',async()=>{const calls=[];const repository={withTransaction:fn=>fn({lockCase:async()=>({id:1}),replaceFactorSnapshot:async(...args)=>calls.push(args),updateComplexity:async(_id,row)=>row,writeAudit:async()=>{}})};const result=await c.assessComplexity({repository,caseId:1,facts,actorId:7,reason:'  reviewed evidence  ',canManage:true});expect(calls[0][4]).toBe('reviewed evidence');expect(result).toMatchObject({complexity_score:c.scoreFacts(facts).score,complexity_class:c.scoreFacts(facts).complexityClass,workload_units:c.scoreFacts(facts).workloadUnits});});
 test('missing assessment reason rejects before opening a repository transaction',async()=>{const repository={withTransaction:jest.fn()};await expect(c.assessComplexity({repository,caseId:1,facts,actorId:7,reason:' ',canManage:true})).rejects.toThrow('reason');expect(repository.withTransaction).not.toHaveBeenCalled();});
});
describe('commercial',()=>{
