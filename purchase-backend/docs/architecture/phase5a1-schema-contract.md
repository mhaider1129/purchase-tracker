...
| `item_uom` | `id` | `uom_code`, `uom_name` | `is_active` | `normalized_uom_code` | none in deployed snapshot | Foundation service reference methods / generic, product, inventory readers |
...
The legacy repository joined product UOM through nonexistent `approved_products.uom_id`. Runtime SQL now uses the deployed `approved_products.product_uom_id`. Reference queries use `category_name`, `manufacturer_name`, and `uom_code`; compatibility `name`/`code` values are output labels only.
