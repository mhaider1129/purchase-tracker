...
Every domain reports `coverage`, `full_cases`, `partial_cases`, `missing_cases`, `legacy_incomplete_cases`, `usable_evidence_cases`, `total_cases`, and server-calculated percentages. Usable evidence is exactly `FULL + PARTIAL`; `MISSING` and `LEGACY_INCOMPLETE` are excluded. `coverage_percent` is `100 × usable_evidence_cases / total_cases`, while `full_coverage_percent` is the separate `100 × full_cases / total_cases`; both use exact server `NUMERIC` arithmetic and are formatted to two decimals. The aggregate state is `FULL` only when every case is full, `PARTIAL` when any usable evidence exists but not every case is full, `LEGACY_INCOMPLETE` when no usable evidence exists but legacy-incomplete cases do, and `MISSING` otherwise (including an empty population).

For activity, only `FULL` and `PARTIAL` cases contribute their known touch, RFQ, and quotation numerators, exactly matching the usable-evidence population. A `PARTIAL` result displays those known counts with a coverage warning. Stored activities belonging to `MISSING` or `LEGACY_INCOMPLETE` cases remain in the individual case timeline, but are excluded from trusted executive KPI aggregation unless later reconciliation explicitly upgrades the case's coverage. With no usable evidence (`MISSING`, or legacy-only), these metrics are `not_available`. Demand counts never depend on activity coverage. Complexity class/PWU aggregates include genuinely assessed cases only and separately report `assessed_cases` and `unassessed_cases`; an unassessed case is `MISSING`, while `LEGACY_INCOMPLETE` identifies a historical case whose assessment cannot be reconstructed. `PARTIAL` is reserved for a genuinely incomplete complexity assessment, not merely an absent assessment. Award value, verified savings, and normalized credit terms use their own source evidence and must not inherit a single optimistic commercial flag.
...
Case status answers **where the procurement is in the lifecycle**; pending root cause answers **why it is waiting**. A lifecycle position alone is never evidence of a cause. In particular, `PO_ISSUED` and a partial goods receipt project `SUPPLIER_FULFILLMENT` with a null cause, not `SUPPLIER_MANUFACTURING`. Authoritative later evidence or a governed, audited override may identify manufacturing, finance payment, international shipment, customs/regulatory, or technical clarification as the reason. Thus the same `SUPPLIER_FULFILLMENT` status can legitimately have any of those governed causes or no known cause.

Durations are independent: submission→full approval; assignment/sourcing start→commercially ready; technical request→technical decision; award→PO issue; PO→dispatch; shipment→delivery; and submission→delivery. The last is explicitly end-to-end, not Supply Chain processing time. `commercially_ready_at` is first set by the authoritative `AWARD_CREATED` event: an award proves that procurement has enough commercial evidence to move forward. An ordinary `RFX_RESPONSE_SUBMITTED` only moves the projection to `COMMERCIAL_EVALUATION` and never sets that timestamp, because one quotation may be insufficient. First-quotation timing, if needed later, must be derived as `MIN(activity_at)` over `QUOTATION_RECEIVED` activities rather than stored in this field. Pre-award cases therefore retain partial cycle-time coverage rather than receiving a fabricated timestamp. Responsibility clocks (`SUPPLY_CHAIN`, `END_USER`, `TECHNICAL`, `SUPPLIER`, `FINANCE`, `LOGISTICS`, `CUSTOMS_REGULATORY`, `APPROVAL`, `OTHER`) require paired event evidence; current coverage is insufficient for a complete attribution report.
