...
async function postCount(input, suppliedClient = null) {
  ...
  return withTransaction(async (client) => {
    ...
    if (count.posted_at) throw new InventoryError('CYCLE_COUNT_ALREADY_POSTED','Cycle count was already posted',409);
    ...
    await client.query(`UPDATE inventory_cycle_counts SET status='POSTED',posted_at=CURRENT_TIMESTAMP,posted_by=$2 WHERE id=$1`,[count.id,input.actor.id]);
