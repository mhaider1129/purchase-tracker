...
function normalizeQuotationLines(lines, requestedItems, currency = 'USD') {
  ...
  return { lines: normalized, total: formatProduct(total) };
}
...
async function submitLinkedRfxResponse({ repository, event, supplierId, submittedBy, bidAmount, notes, responseData, lines }) {
  ...
  const response = await repository.insertResponse({ rfx_id: event.id, request_id: event.request_id, supplier_id: supplierId, submitted_by: submittedBy, bid_amount: quotation.total, notes, response_data: responseData });
  ...
  return { ...response, bid_amount: quotation.total, quotation_total: quotation.total, items: persistedLines };
}
