...
class StockItemMappingService {
  ...
  async list(filters={}) { const p=pagination(filters); const rows=await new StockItemMappingRepository(this.db).list({status:filters.status,stockItemId:filters.stock_item_id,genericItemId:filters.generic_item_id},p); return {data:rows.map(({total_count,...row})=>row),pagination:{page:p.page,limit:p.limit,total:rows[0]?.total_count||0}}; }
  ...
  async history(stockItemId,filters={}) { return this.list({...filters,stock_item_id:stockItemId}); }
