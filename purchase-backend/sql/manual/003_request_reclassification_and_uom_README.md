...
Migration 003 is self-contained for the Phase 2 route identity and supersession schema. It safely guarantees `requests.approval_route_snapshot`, `requests.approval_route_snapshot_id`, `approvals.route_snapshot_id`, `approval_route_version`, and the supersession metadata. The request snapshot columns are included because no checked-in numbered/manual migration 002 guarantees them. SQL 002 concerns such as notification outbox and approval timestamps are intentionally not duplicated. No UOM DDL is needed because the current UOM correction is application-only.
...
The unique index includes the approver: `(request_id, approval_route_version, approval_level, approver_id)`. Multiple active Pending members at one current level are valid and are not normalized away. The migration deliberately creates no one-active-approval-per-request invariant.