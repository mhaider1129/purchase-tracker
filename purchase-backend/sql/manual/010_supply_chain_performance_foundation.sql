...
BEGIN
  ...
  WITH required_columns(table_name, column_name) AS (VALUES
    ...
    ('procurement_cases_one_active_item_uq','CREATE UNIQUE INDEX procurement_cases_one_active_item_uq%ON public.procurement_cases USING btree (requested_item_id) WHERE (closed_at IS NULL)'),
    ('procurement_cases_scope_idx','CREATE INDEX procurement_cases_scope_idx%ON public.procurement_cases USING btree (institute_id, department_id, assigned_buyer_id, opened_at)'),
    ('procurement_cases_pipeline_idx','CREATE INDEX procurement_cases_pipeline_idx%ON public.procurement_cases USING btree (case_status, pending_root_cause) WHERE (closed_at IS NULL)'),
    ('procurement_case_activities_idempotency_uq','CREATE UNIQUE INDEX procurement_case_activities_idempotency_uq%ON public.procurement_case_activities USING btree (idempotency_key) WHERE (idempotency_key IS NOT NULL)'),
    ('procurement_case_activities_timeline_idx','CREATE INDEX procurement_case_activities_timeline_idx%ON public.procurement_case_activities USING btree (procurement_case_id, activity_at DESC)'),
    ('procurement_case_activities_supplier_idx','CREATE INDEX procurement_case_activities_supplier_idx%ON public.procurement_case_activities USING btree (supplier_id, activity_type) WHERE (supplier_id IS NOT NULL)'),
    ('procurement_value_events_case_currency_idx','CREATE INDEX procurement_value_events_case_currency_idx%ON public.procurement_value_events USING btree (procurement_case_id, value_type, currency)')
  ...
  IF mismatches IS NULL THEN
    RAISE NOTICE 'SQL_010_ALREADY_APPLIED_COMPATIBLE';
    RETURN;
  END IF;
