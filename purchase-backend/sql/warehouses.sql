...
CREATE TABLE IF NOT EXISTS public.warehouses (
  ...
  institute_id INTEGER REFERENCES public.institutes(id) ON DELETE SET NULL,
...
ALTER TABLE IF EXISTS public.warehouses
  DROP CONSTRAINT IF EXISTS warehouses_institute_id_fkey;

ALTER TABLE IF EXISTS public.warehouses
  ADD CONSTRAINT warehouses_institute_id_fkey
...
INSERT INTO public.warehouses (name, department_id, institute_id, description)
SELECT d.name, d.id, d.institute_id, 'Migrated from departments table'
...
UPDATE public.warehouses w
   SET institute_id = d.institute_id
  FROM public.departments d
 WHERE w.department_id = d.id
   AND w.institute_id IS NULL;
